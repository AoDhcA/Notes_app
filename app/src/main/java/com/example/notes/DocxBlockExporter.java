...
public class DocxBlockExporter {
    ...
    private static void exportFormattedText(XWPFParagraph paragraph, String htmlContent) {
        ...
        try {
            // Декодирование HTML-сущности перед парсингом
            ...
            List<TextSegment> segments = parseHtmlWithLineBreaks(decodedHtml); // парсер HTML
            ...
            // Фолбэк: сохранение как обычный текст
            XWPFRun run = paragraph.createRun();
    ...
    private static List<TextSegment> parseHtmlWithLineBreaks(String html) {
        ...
        // Выражение для поиска всех элементов
        // 1) теги <font> с содержимым, 2) теги <br>, 3) произвольный текст
        ...
        while (matcher.find()) {
            ...
            if (match.startsWith("<font")) {
                ...
                if (fontMatcher.find()) {
                    ...
                    // Разделение содержимого тега <font> по <br>
                    ...
                    for (int i = 0; i < parts.length; i++) {
                        ...
                        // Перенос строки между частями (кроме последней)
                        if (i < parts.length - 1) {
                ...
                // Если предыдущий сегмент не был пробелом, добавляется один пробел
                if (segments.size() > 0) {
        ...
        // Обработка оставшегося текст, если есть
